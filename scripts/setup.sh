#!/bin/bash
# =============================================================================
# OpenTracker - Quick Docker Setup
# =============================================================================
...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║          OpenTracker - Docker Setup                       ║${NC}"
echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""

# Check if .env.example exists
if [[ ! -f "$PROJECT_DIR/.env.example" ]]; then
    log_error ".env.example not found. Are you in the OpenTracker directory?"
fi
...
# Database
sed_inplace "s|^DATABASE_URL=.*|DATABASE_URL=postgres://tracker:$DB_PASSWORD@pgbouncer:6432/opentracker|" "$PROJECT_DIR/.env"
