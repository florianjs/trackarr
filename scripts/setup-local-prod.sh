#!/bin/bash
# =============================================================================
# OpenTracker - Local Production Setup Script
# =============================================================================
...
echo -e "${GREEN}${BOLD}"
echo "  ┌──────────────────────────────────────────────────────────────┐"
echo "  │       OpenTracker - Local Production Setup                   │"
echo "  │                    with OrbStack                             │"
echo "  └──────────────────────────────────────────────────────────────┘"
echo -e "${NC}"
...
cat > "$ENV_FILE" << EOF
# =============================================================================
# OpenTracker - Local Production Environment
...
DB_HOST=postgres
DB_PORT=5432
DATABASE_URL=postgres://tracker:${DB_PASSWORD}@postgres:5432/opentracker
...
docker stop $(docker ps -q --filter "name=opentracker") 2>/dev/null || true
docker rm $(docker ps -aq --filter "name=opentracker") 2>/dev/null || true
...
# Wait for PostgreSQL
for i in {1..30}; do
    if docker compose -f docker-compose.local.yml --env-file "$ENV_FILE" exec -T postgres pg_isready -U tracker -d opentracker &> /dev/null; then
...
if command -v orb &> /dev/null; then
    ...
    echo -e "  Frontend:     ${CYAN}http://opentracker.orb.local:3000${NC}"
    echo -e "  Grafana:      ${CYAN}http://opentracker-grafana.orb.local:3000${NC}"
    echo ""
fi
